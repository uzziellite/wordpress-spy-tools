...
def extract_theme_name(url):
    """Extract theme name from WordPress theme URL"""
    match = re.search(r'/wp-content/themes/([^/]+)', url)
    return match.group(1) if match else url

def display_results_table(results):
    """Display results in a formatted table, filtering out 404 status code"""
    table = Table(title="Themes Discovered", box=box.ROUNDED)
    ...
    for result in results:
        ...
        theme_name = extract_theme_name(result['url'])
...
def process_urls(checker, file_path, max_workers=10):
    ...
    try:
        with open(file_path, 'r') as file:
            paths = [line.strip().replace('plugins', 'themes') for line in file if line.strip()]
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as ex:
            ...
            with tqdm(total=len(futures), desc="Searching Themes: ", unit="url") as pbar:
                progress_bar = pbar
...
def main():
    parser = argparse.ArgumentParser(description='Concurrent WordPress Theme Checker')
    ...
    try:
        ...
        console.print(f"\nStarting Theme Search with base URL: [cyan]{args.base_url}[/cyan]\n")
